...
def intraday_trade_bot_1(p1, p2, p1_pred, p2_pred):
    ...
    correct = 0
    incorrect = 0
    for i in range(len(p1)-30):
        if p2_pred[i] > (p1_pred[i]):
            profit-= p1[i]
            stock+= 1
            trades+= 1
            if p2[i] > (p1[i]):
    ...
    return stock, trades, profit, correct, incorrect
...
for method in methods:
    if method == 'ridge':
        for symbol in symbol_list:
            ...
            stocks_eod, trades_eod, profit_eod, correct_eod, incorrect_eod = [], [], [], [], []
            for day in trading_days:
                ...
                stock_day, stock_traded_day, profit_day, correct_day, incorrect_day = intraday_trade_bot_1(y1_test, y2_test, y1_pred, y2_pred)
                stocks_eod.append(stock_day)
                trades_eod.append(stock_traded_day)
                profit_eod.append(profit_day)
                correct_eod.append(correct_day)
                incorrect_eod.append(incorrect_day)
                ## create numpy array lol...
            print_graphs = 0
            if print_graphs is 1:
                #print('stocks traded per day')
                #print(trades_eod)
                print('avg stocks traded per day, median and mode')
                print(np.median(trades_eod))
                print(np.mean(trades_eod))

                #print('profit per day')
                #print(profit_eod)
                print('avg profit per day, median and mode')
                print(np.median(profit_eod))
                print(np.mean(profit_eod))

                #print('correct predictions per day')
                #print(correct_eod)
                print('avg correct predictions per day, median and mean')
                print(np.median(correct_eod))
                print(np.mean(correct_eod))

                #print('incorrect predictions per day')
                #print(incorrect_eod)
                print('avg incorrect predictions per day, median and mean')
                print(np.median(incorrect_eod))
                print(np.mean(incorrect_eod))

                correct_to_total_eod = np.nan_to_num(np.divide(correct_eod, trades_eod))
                #print('ratio of correct to total predictions per day')
                #print(correct_to_total_eod)
                print('avg ratio of correct to total predictions per day, median and mean')
                print(np.median(correct_to_total_eod))
                print(np.mean(correct_to_total_eod))

                # plot end of day tallies
                plt.figure(figsize=(16,9))
                plt.bar(trading_days, trades_eod/np.mean(trades_eod))
                plt.bar(trading_days,profit_eod/np.mean(profit_eod))
                plt.bar(trading_days,correct_eod/np.mean(correct_eod))
                plt.bar(trading_days,incorrect_eod/np.mean(incorrect_eod))
                plt.bar(trading_days,correct_to_total_eod/np.mean(correct_to_total_eod))
                plt.xlabel('Time [day]')
                plt.ylabel('Data (corrected by mean)')
                plt.legend(('trades ','profit','correct prediction', 'incorrect prediction', 'ratio of correct to total prediction'))
                plt.title('Ridge Regression Prediction and Trading Results')
                #plt.show()
                plt.savefig('ridge_all.png')

                plt.figure(figsize=(16,6))
                plt.bar(trading_days,trades_eod)
                plt.axhline(y=np.mean(trades_eod))
                plt.xlabel('Time [day]')
                plt.ylabel('trades per day')
                plt.legend(('trades per day','avg trades per day'))
                plt.title('Ridge Regression Prediction and Trading, Trades per Day')
                #plt.show()
                plt.savefig('ridge_trades_eod.png')

                plt.figure(figsize=(16,6))
                plt.bar(trading_days,profit_eod)
                plt.axhline(y=np.mean(profit_eod))
                plt.xlabel('Time [day]')
                plt.ylabel('profit per day')
                plt.legend(('profit per day','avg profit per day'))
                plt.title('Ridge Regression Prediction and Trading, Profit per Day')
                #plt.show()
                plt.savefig('ridge_profit_eod.png')          

                plt.figure(figsize=(16,6))
                plt.bar(trading_days,correct_eod)
                plt.axhline(y=np.mean(correct_eod))
                plt.xlabel('Time [day]')
                plt.ylabel('correct predictions per day')
                plt.legend(('correct predictions per day','avg correct predictions per day'))
                plt.title('Ridge Regression Prediction and Trading, Correct Predictions per Day')
                #plt.show()
                plt.savefig('ridge_correct_eod.png')

                plt.figure(figsize=(16,6))
                plt.bar(trading_days,incorrect_eod)
                plt.axhline(y=np.mean(incorrect_eod))
                plt.xlabel('Time [day]')
                plt.ylabel('incorrect predictions per day')
                plt.legend(('incorrect predictions per day','avg incorrect predictions per day'))
                plt.title('Ridge Regression Prediction and Trading, Incorrect Predictions per Day')
                #plt.show()
                plt.savefig('ridge_incorrect_eod.png')

                plt.figure(figsize=(16,6))
                plt.bar(trading_days,correct_to_total_eod)
                plt.axhline(y=np.mean(correct_to_total_eod))
                plt.xlabel('Time [day]')
                plt.ylabel('ratio of correct to incorrect per day')
                plt.legend(('ratio of correct to incorrect per day','avg ratio of correct to incorrect per day'))
                plt.title('Ridge Regression Prediction and Trading, Ratio of Correct to Incorrect per Day')
                #plt.show()
                plt.savefig('ridge_correct_to_total_eod.png')


    if method == 'lstm':
            price_by_symbol[symbol].index = price_by_symbol[symbol]['time']
            trades_eod, profit_eod, correct_eod, incorrect_eod = [], [], [], []
            for day in trading_days:
                ...
                param = normalize(param)
                param.shape
                x_train, x_test, y_train, y_test = split_train_test_LSTM(param,num_of_bins_per_nn,train_test_ratio=3/5)
                ...
                length_train = int(length*3/5)
                print('length')
                print(length)
                print('lengthtrain')
                print(length_train)
                print('length_tests_new')
                print(len(tests_new))
            ...
            if print_graphs is 1:
                ...
                plt.ylabel('Data (corrected by mean)')
                plt.legend(('trades ','profit','correct prediction', 'incorrect prediction', 'ratio of correct to total prediction'))
                plt.title('LSTM Prediction and Trading Results')
                #plt.show()
                ...
                plt.ylabel('trades per day')
                plt.legend(('trades per day','avg trades per day'))
                plt.title('LSTM Regression Prediction and Trading, Trades per Day')
                #plt.show()
                plt.savefig('LSTM_trades_eod.png')
                ...
                plt.ylabel('profit per day')
                plt.legend(('profit per day','avg profit per day'))
                plt.title('LSTM Regression Prediction and Trading, Profit per Day')
                #plt.show()
                plt.savefig('LSTM_profit_eod.png')          
                ...
                plt.ylabel('correct predictions per day')
                plt.legend(('correct predictions per day','avg correct predictions per day'))
                plt.title('LSTM Regression Prediction and Trading, Correct Predictions per Day')
                #plt.show()
                plt.savefig('LSTM_correct_eod.png')
                ...
                plt.ylabel('incorrect predictions per day')
                plt.legend(('incorrect predictions per day','avg incorrect predictions per day'))
                plt.title('LSTM Regression Prediction and Trading, Incorrect Predictions per Day')
                #plt.show()
                plt.savefig('LSTM_incorrect_eod.png')
                ...
                plt.ylabel('ratio of correct to incorrect per day')
                plt.legend(('ratio of correct to incorrect per day','avg ratio of correct to incorrect per day'))
                plt.title('LSTM Regression Prediction and Trading, Ratio of Correct to Incorrect per Day')
                #plt.show()
                plt.savefig('LSTM_correct_to_total_eod.png')
